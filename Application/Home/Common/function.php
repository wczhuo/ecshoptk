...
function get_user_bonus($user_id = 0){
    ...
    $row=M('user_bonus')->alias('ub')->field('SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count')>join(C('DB_PREFIX').'bonus_type b ON b.user_id= a.id')->where(array('ub.user_id = '.$user_id.' AND ub.bonus_type_id = bt.type_id AND ub.order_id = 0'))->find();
...
function get_vote($id = ''){
    ...
    if (empty($id)){
        $time = gmtime();
        $vote_arr=M('vote')->field('vote_id, vote_name, can_multi, vote_count, RAND() AS rnd')->where(array('start_time <= '.$time,'end_time >= '.$time))->order('rnd')->find();
    }else{
        $sql = 'SELECT vote_id, vote_name, can_multi, vote_count' .
               ' FROM ' . $GLOBALS['ecs']->table('vote').
               " WHERE vote_id = '$id'";
        $vote_arr=M('vote')->field('vote_id, vote_name, can_multi, vote_count')->where(array('vote_id'=>$id))->find();
    }

    if ($vote_arr !== false && !empty($vote_arr)){
        /* 通过调查的ID,查询调查选项 */
        $res=M('vote')->alias('v')->join(C('DB_PREFIX').'vote_option b ON b.user_id= a.id')->field('v.*, o.option_id, o.vote_id, o.option_name, o.option_count')->where(array('o.vote_id = v.vote_id AND o.vote_id = '.$vote_arr['vote_id']))->order('o.option_order ASC, o.option_id DESC')->select();

        /* 总票数 */
        $option_num=M('vote_option')->field('SUM(option_count) AS all_option')->where(array('vote_id = '.$vote_arr['vote_id']))->group('vote_id')->count();
 ...
 * @return  array
 */
function ecs_load_config(){
    $arr = array();

    $data = read_static_cache('shop_config');
    if ($data === false){
        $res = M('shop_config')->field('code, value')->where(array('parent_id > 0'))->select($sql);
...
function get_brands($cat = 0, $app = 'brand'){
    ...
    $row = M('brand')->alias('b')->join(C('DB_PREFIX').'goods g ON g.brand_id= b.brand_id')->field("b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag")->where(array("g.brand_id = b.brand_id $children AND is_show = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 "))->group('b.brand_id HAVING goods_num > 0')->order('tag DESC, b.sort_order ASC')->select();
...
function get_promotion_info($goods_id = ''){
    ...
    $gmtime = gmtime();
    $WhereList4GoodsAct=array('is_finished'=>0,'start_time <='.$gmtime,'end_time >='.$gmtime);
    if(!empty($goods_id)){
        $WhereList4GoodsAct['goods_id'] =$goods_id;
    }
    $res=M('goods_activity')->field('act_id, act_name, act_type, start_time, end_time')->where($WhereList4GoodsAct)->select();
    ...
    $WhereList4FavAct=array('start_time <='.$gmtime,'end_time >='.$gmtime);
    
    if(!empty($goods_id)){
        $WhereList4FavAct[] = " CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'";
    }
    $res = M('favourable_activity')->field('act_id, act_range, act_range_ext, act_name, start_time, end_time')->where($WhereList4FavAct)->select();
    ...
    if(empty($goods_id))
    {
        ...
        $row = M('goods')->field('cat_id, brand_id')->where(array('goods_id'=>$goods_id))->find();
...
{
    $res = M('article')->alias('a')->join(C('DB_PREFIX').'article_cat ac ON ac.cat_id= a.cat_id')->field('a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type, ac.cat_id, ac.cat_name')->where(array('a.is_open = 1 AND a.cat_id = ac.cat_id AND ac.cat_type = 1'))->order('a.article_type DESC, a.add_time DESC')->limit(C('article_number'))->select();
