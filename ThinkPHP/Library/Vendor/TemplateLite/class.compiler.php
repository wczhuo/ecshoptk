	...
	function _parse_function($function, $modifiers, $arguments)
	{
		switch ($function) {
            ...
            case 'member':
				...
				/*if (!isset($_args['from']))
				{
					$this->trigger_error("missing 'from' attribute in 'member'", E_USER_ERROR, __FILE__, __LINE__);
				}*/
				if (!isset($_args['value']) && !isset($_args['item']))
				{
					$this->trigger_error("missing 'value' attribute in 'member'", E_USER_ERROR, __FILE__, __LINE__);
				}
				if (isset($_args['value']))
				{
					$_args['value'] = $this->_dequote($_args['value']);
