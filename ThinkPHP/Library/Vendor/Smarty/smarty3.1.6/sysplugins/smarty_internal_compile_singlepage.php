    ...
    public function compile($args, $compiler, $parameter)
    {
        ...
        if($_attr['ispage']=='1'){
            if(isset($_attr['pagesize'])&&is_numeric($_attr['pagesize'])&&isset($_attr['pagename'])){
                ...
                $OutputStr.='$count = M("member")->where($Where)->count();$Page = new \Think\Page($count,'.$_attr['pagesize'].');$show = $Page->show();$_smarty_tpl->tpl_vars['.$_attr['pagename'].']->value=$show;';
                $OutputStr.='$MemberList=M("member")->where($Where)->page(((I(\'p\')&&is_numeric(I(\'p\')))?I(\'p\'):\'1\').",'.$_attr['pagesize'].'")->select();';
            }else{
                $OutputStr.='$MemberList=M("member")->where($Where)->select();';
            }
        }else if(isset($_attr['limit'])&&is_numeric($_attr['limit'])){
            $OutputStr.='$MemberList=M("member")->where($Where)->limit('.$_attr['limit'].')->select();';
        }else{
            $OutputStr.='$MemberList=M("member")->where($Where)->select();';
        }
