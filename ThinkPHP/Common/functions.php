...
/**
 * Summary of ArrayToString
 * @param mixed $object 
 * @param mixed $withKey 
 * @param mixed $two 
 * @return mixed
 */
function ArrayToString($object,$withKey=false,$two=false,$level=1,$Key){
	...
	if ($objType=='array') {
	    ...
	    foreach ($object as $objkey=>$objv) {
			...
			if ($vtype=='integer') {
                ...
                $objstring .=ArrayToString($objv,$withKey,false,$level+1).",";
...
/**
 * Summary of ConvertEncodingToUTF8
 * @param mixed $string 
 * @return mixed
 */
function ConvertEncodingToUTF8($string){
    $InputCoding=strtolower(mb_detect_encoding($string));
    if(in_array($InputCoding,array('utf8','utf_8'))){
        return $string;
    }else{
        return mb_convert_encoding($string,'utf8',$InputCoding);
    }
}

/**
 * Summary of ConvertEncodingToGBK
 * @param mixed $string 
 * @return mixed
 */
function ConvertEncodingToGBK($string){
    $InputCoding=strtolower(mb_detect_encoding($string));
    if($InputCoding=='gbk'){
        return $string;
    }else{
        return mb_convert_encoding($string,'gbk',$InputCoding);
    }
}

function ConvertEncodingToGB2312($string){
    $InputCoding=strtolower(mb_detect_encoding($string));
    if($InputCoding=='gb2312'){
        return $string;
    }else{
        return mb_convert_encoding($string,'gb2312',$InputCoding);
    }
}
 ...
 * @return mixed
 */
function MakeDirectory($directoryName, $directoryMode = 0777){
    if (is_dir($directoryName) || @mkdir($directoryName, $directoryMode)){
        return TRUE;
    }
    if (!mkdirs(dirname($directoryName), $directoryMode)){
        return FALSE;
    }
